...
export default function GreekTubePlayer() {
  ...
  useEffect(()=>{
    if(!hydrated)return;
    const missing=state.videos.filter(video=>video.metadataVersion!==2);
    ...
    void Promise.all(missing.map(async video=>{
      ...
      setState(current=>({...current,videos:current.videos.map(video=>{
        ...
        return {...video,title:metadata.title||video.title,originalTitle:metadata.originalTitle||video.originalTitle,channel:metadata.channel||video.channel,duration:metadata.duration||video.duration,description:metadata.description||video.description,speakerName:metadata.speakerName||video.speakerName,category:metadata.category||video.category,tags:metadata.tags?.length?Array.from(new Set([...video.tags,...metadata.tags])):video.tags,metadataVersion:2};
      })}));
    });
...
function Brand({home}:{home:()=>void}){return <button className="brand brand-home" aria-label="Αρχική σελίδα" onClick={home}><span className="brand-mark"><i>≡</i>▶</span><span>GreekTube <b>Subs</b></span><small className="brand-version">ver 4.4</small></button>;}
