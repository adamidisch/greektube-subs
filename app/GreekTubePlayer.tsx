    ...
    <main className={`app-shell viewer player-screen-transition ${checkingReady?"player-is-opening":""} ${settingsFromPlayer?"viewer-settings-open":""}`} aria-hidden={settingsFromPlayer?true:undefined}>
      ...
      {checkingReady&&<section className="watch-layout player-only player-opening-shell" role="status" aria-live="polite" aria-label="Φόρτωση βίντεο">
        <div className="watch-main">
          <div className="mobile-video-byline player-owner-strip player-opening-byline"><div className="player-owner-copy"><i aria-hidden="true"/><span><strong>{displaySpeakerName}</strong>{displaySpeakerRole&&<small>{displaySpeakerRole}</small>}</span></div><span aria-hidden="true"><i/><i/></span></div>
          <div className="sticky-player">
            ...
            <div className="player-opening-controls player-opening-revamp" aria-hidden="true">
              <div className="player-opening-primary-row"><i/><i/><b/><i/><i/></div>
              <div className="player-opening-secondary-row"><i/><i/><i/><i/></div>
            </div>
          </div>
      ...
      {!checkingReady&&!loading&&captions&&<>
        <section className={`watch-layout player-ready-transition ${transcriptOpen?"transcript-open":"player-only"}`}>
          <div className="watch-main">
            <div className="mobile-video-byline player-owner-strip">
              <div className="player-owner-copy"><i aria-hidden="true"/><span><strong>{displaySpeakerName}</strong>{displaySpeakerRole&&<small>{displaySpeakerRole}</small>}</span></div>
              <span className="player-owner-actions"><button type="button" aria-label="Επεξεργασία βίντεο" onClick={()=>void requestEdit(selected)}>✎</button><button type="button" aria-label={selected.favorite?"Αφαίρεση από τα αγαπημένα":"Προσθήκη στα αγαπημένα"} className={selected.favorite?"active":""} onClick={()=>patchVideo(selected.id,{favorite:!selected.favorite})}>♡</button></span>
            </div>
            <div className="sticky-player" onContextMenu={e=>{e.preventDefault();beginMoment();}}>
              ...
              <div className="player-actions player-tools player-control-deck">
                <div className="player-control-primary-row" role="group" aria-label="Έλεγχος αναπαραγωγής">
                  <button className="fullscreen-toggle fullscreen-primary revamp-icon-control" aria-label="Πλήρης οθόνη" onClick={event=>{event.currentTarget.blur();void toggleFullscreen();}}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" aria-hidden="true"><path d="M8 3H5a2 2 0 00-2 2v3m18 0V5a2 2 0 00-2-2h-3M3 16v3a2 2 0 002 2h3m11 0h3a2 2 0 002-2v-3"/></svg></button>
                  <button className="skip-button revamp-icon-control" aria-label="Πίσω 10 δευτερόλεπτα" onClick={()=>skip(-10)}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><path d="M15 6l-6 6 6 6"/></svg><small>10</small></button>
                  <button className="play-toggle revamp-play-control" aria-label={isPlaying?"Παύση":"Αναπαραγωγή"} onClick={togglePlayback}>{isPlaying?"Ⅱ":"▶"}</button>
                  <button className="skip-button revamp-icon-control" aria-label="Μπροστά 10 δευτερόλεπτα" onClick={()=>skip(10)}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><path d="M9 6l6 6-6 6"/></svg><small>10</small></button>
                  <div className="volume-control revamp-volume-control">
                    <button className="volume-toggle revamp-icon-control" aria-label={isMuted||volume===0?"Ενεργοποίηση ήχου":"Ρύθμιση ή σίγαση ήχου"} onClick={()=>setVolumeSliderOpen(open=>!open)}>
                ...
                <div className="player-secondary-actions player-control-secondary-row">
                  <label className="speed-control revamp-seg-control" aria-label="Ταχύτητα αναπαραγωγής"><select value={state.settings.speed} onChange={event=>{const next=Number(event.target.value);setState(current=>({...current,settings:{...current.settings,speed:next}}));currentPlayer()?.setPlaybackRate(next);}}>{PLAYBACK_SPEEDS.map(speed=><option key={speed} value={speed}>{speed}×</option>)}</select><span>Ταχύτητα</span></label>
                  <button className="moment-save revamp-seg-control" onClick={()=>beginMoment()}>{moments.length>0&&<span className="moment-count">{moments.length}</span>}<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" aria-hidden="true"><path d="M12 5v14M5 12h14"/></svg><span>Στιγμή</span></button>
                  <button className={`transcript-toggle revamp-seg-control ${transcriptOpen?"active":""}`} aria-label={transcriptOpen?"Κλείσιμο κειμένου μεταγραφής":"Άνοιγμα κειμένου μεταγραφής"} aria-pressed={transcriptOpen} onClick={()=>setTranscriptOpen(value=>!value)}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true"><path d="M5 6h14M5 12h14M5 18h9"/></svg><span>Κείμενο</span></button>
                  <div className={`subtitle-cc-control revamp-seg-control ${state.settings.subtitles?"active":""}`}>
                    <button className={`cc-toggle ${state.settings.subtitles?"active":""}`} aria-label="Επιλογές υποτίτλων" aria-expanded={subtitleMenuOpen} onClick={()=>setSubtitleMenuOpen(open=>!open)}><svg className="cc-card-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true"><rect x="3" y="6" width="18" height="12" rx="2"/><path d="M7 10h2M13 10h4M7 14h6"/></svg><span className="cc-label">Υπότιτλοι</span></button>
            ...
            <section className="video-heading player-info-card">
              <small className="video-meta-kicker"><span className="video-category-label" data-category={selected.category}>{CATEGORY_LABELS[selected.category]}</span></small>
              <h1 className="player-greek-title">{isGreekTitle(selected.title)?selected.title:isGreekTitle(captions.title)?captions.title:"Βίντεο με ελληνικούς υπότιτλους"}</h1>
              {(selected.originalTitle||captions.originalTitle||englishTitle(selected))&&<a className="player-original-title" href={sourceVideoUrl} target="_blank" rel="noreferrer" title="Άνοιγμα αρχικού βίντεο στο YouTube">{selected.originalTitle||captions.originalTitle||englishTitle(selected)} ↗</a>}
              <div className="video-source-row"><span>ΠΗΓΗ</span>{sourceChannelUrl?<a href={sourceChannelUrl} target="_blank" rel="noreferrer" title="Άνοιγμα καναλιού στο YouTube">{selected.channel} ↗</a>:<strong>{selected.channel}</strong>}</div>
              <p className={`mobile-video-description ${descriptionExpanded?"expanded":""}`}>{mobileSummary(selected,captions)}</p>
              <button type="button" className={`description-toggle ${descriptionExpanded?"open":""}`} aria-expanded={descriptionExpanded} onClick={()=>setDescriptionExpanded(expanded=>!expanded)}>{descriptionExpanded?"Λιγότερα":"Περισσότερα"}<span aria-hidden="true">⌄</span></button>
            </section>
            <section className="moments"><div className="section-title"><h2>Αποθηκευμένες στιγμές</h2><small>{moments.length}</small></div>{moments.length===0?<p className="muted">Δεν έχεις αποθηκεύσει στιγμές.</p>:moments.map(m=><article className="moment" key={m.id} onClick={()=>seek(m.time)}><time>{clock(m.time)}</time><div><strong>{m.note}</strong><p>{m.excerpt}</p></div><div className="moment-actions"><button onClick={e=>{e.stopPropagation();seek(m.time)}}>Αναπαραγωγή</button><button onClick={e=>{e.stopPropagation();void copyMoment(m)}}>Αντιγραφή συνδέσμου</button><button onClick={e=>{e.stopPropagation();void shareMoment(m)}}>Κοινοποίηση</button><button onClick={e=>{e.stopPropagation();setState(s=>({...s,moments:s.moments.filter(x=>x.id!==m.id)}))}}>Διαγραφή</button></div></article>)}</section>
            {guideItems.length>0&&<section className={`video-guide editorial-guide ${guideOpen?"open":""}`}><button type="button" className="guide-toggle" aria-expanded={guideOpen} onClick={()=>setGuideOpen(open=>!open)}><span><small>{selected.creatorChapters?.length?"ΚΕΦΑΛΑΙΑ YOUTUBE":"ΠΕΡΙΕΧΟΜΕΝΟ"}</small><strong>Οδηγός βίντεο</strong></span><span className="guide-toggle-meta">{guideItems.length?`${guideItems.length} βασικά σημεία`:"Υπό επιμέλεια"}<i aria-hidden="true">⌄</i></span></button>{guideOpen&&<div className="guide-content"><p className="guide-intro">{selected.creatorChapters?.length?"Κεφάλαια που έχει ορίσει ο δημιουργός του βίντεο στο YouTube.":"Επιλεγμένα σημεία με σύντομη περίληψη και πλαίσιο ώστε να πηγαίνεις κατευθείαν στην ουσία."}</p>{guideItems.length?<div className="guide-list editorial-guide-list">{guideItems.map((item,index)=><button key={`${item.time}-${index}`} onClick={()=>seek(item.time)}><time>{clock(item.time)}</time><span><strong>{item.title}</strong>{item.summary&&<small>{item.summary}</small>}{item.comment&&<em><b>Σχόλιο</b>{item.comment}</em>}</span><i aria-hidden="true">▶</i></button>)}</div>:<div className="guide-empty">Ο οδηγός δεν έχει ακόμη επιμεληθεί για αυτό το βίντεο.</div>}</div>}</section>}
