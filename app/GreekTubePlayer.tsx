            ...
            <div className="sticky-player" onContextMenu={e=>{e.preventDefault();beginMoment();}}>
              ...
              <div className="player-actions">
                <div className="player-toolbar">
                  <section className="control-section primary-control-section"><small>Αναπαραγωγή και υπότιτλοι</small><div className="primary-control-row"><div className="playback-controls" role="group" aria-label="Έλεγχος αναπαραγωγής"><button className="skip-button" aria-label="Πίσω 10 δευτερόλεπτα" onClick={()=>skip(-10)}>≪ <span>10s</span></button><button className="play-toggle" aria-label={isPlaying?"Παύση":"Αναπαραγωγή"} onClick={togglePlayback}>{isPlaying?"Ⅱ":"▶"}</button><button className="skip-button" aria-label="Μπροστά 10 δευτερόλεπτα" onClick={()=>skip(10)}><span>10s</span> ≫</button></div><div className="subtitle-size-picker" role="group" aria-label="Μέγεθος ελληνικών υποτίτλων">{[16,19,22].map((size,index)=><button key={size} className={state.settings.subtitleSize===size?"active":""} style={{fontSize:`${10+index*3}px`}} aria-label={index===0?"Μικροί υπότιτλοι":index===1?"Μεσαίοι υπότιτλοι":"Μεγάλοι υπότιτλοι"} aria-pressed={state.settings.subtitleSize===size} onClick={()=>setState(current=>({...current,settings:{...current.settings,subtitleSize:size,subtitleSizeVersion:2}}))}>A</button>)}</div></div></section>
                </div>
                <section className="control-section action-section"><small>Ενέργειες</small><div className="player-secondary-actions"><button className="fullscreen-toggle" aria-label="Πλήρης οθόνη" onClick={()=>void toggleFullscreen()}><span aria-hidden="true">↗</span> Πλήρης οθόνη</button><button className="moment-save" onClick={()=>beginMoment()}><span aria-hidden="true">＋</span> Αποθήκευση στιγμής</button><button className="transcript-toggle" onClick={()=>setTranscriptOpen(value=>!value)}>{transcriptOpen?"Κλείσιμο μεταγραφής":"Άνοιγμα μεταγραφής"}</button></div></section>
              </div>
            </div>
...
function Brand({home}:{home:()=>void}){return <button className="brand brand-home" aria-label="Αρχική σελίδα" onClick={home}><span className="brand-mark"><i>≡</i>▶</span><span>GreekTube <b>Subs</b></span><small className="brand-version">ver 4.7</small></button>;}
