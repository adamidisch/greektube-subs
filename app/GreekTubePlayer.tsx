...
export default function GreekTubePlayer() {
  ...
  useEffect(()=>{
    if(!hydrated)return;
    const missing=state.videos.filter(video=>!video.duration);
    ...
    void Promise.all(missing.map(async video=>{
      try{
        ...
        const metadata=await response.json() as {id?:string;duration?:number};
        return metadata.id&&metadata.duration?{id:metadata.id,duration:metadata.duration}:null;
      }catch{return null;}
    })).then(results=>{
      if(cancelled)return;
      const durations=new Map(results.filter((result):result is {id:string;duration:number}=>Boolean(result)).map(result=>[result.id,result.duration]));
      if(!durations.size)return;
      setState(current=>({...current,videos:current.videos.map(video=>durations.has(video.id)?{...video,duration:durations.get(video.id)!}:video)}));
    });
  ...
  async function openVideo(video:Video,start?:number,showTranscript=false){
    ...
    const localRecord=localStorage.getItem(`greektube-transcript:${video.id}:v2`);
    if(localRecord){
      try{
        ...
        if(isCompleteGreekTranscript(cached,video.duration)){
          ...
          patchVideo(video.id,{title:cached.title,originalTitle:cached.originalTitle||englishTitle(video),channel:cached.channel,captions:cached.cues,speakerName:cached.speaker?.name,lastWatched:new Date().toISOString()});
          ...
          void fetch("/api/captions",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:video.url})}).then(async response=>{
            ...
            localStorage.setItem(`greektube-transcript:${video.id}:v2`,JSON.stringify(refreshed));
        ...
        <section className={`watch-layout ${transcriptOpen?"transcript-open":"player-only"}`}>
          <div className="watch-main">
            <div className="sticky-player" onContextMenu={e=>{e.preventDefault();beginMoment();}}>
              <div className={`video-frame ${isPseudoFullscreen?"pseudo-fullscreen":""}`} ref={fullscreenHost}><div ref={playerHost}/>{state.settings.subtitles&&active>=0&&<div className={`subtitles ${state.settings.subtitlePosition}`} style={{"--subtitle-size":`${state.settings.subtitleSize}px`,background:`rgba(0,0,0,${state.settings.opacity})`} as CSSProperties}>{state.settings.subtitleMode==="en"?(captions.englishCues?.[active]?.text||captions.cues[active]?.text):state.settings.subtitleMode==="dual"?<><span>{captions.cues[active]?.text}</span>{captions.englishCues?.[active]?.text&&<small>{captions.englishCues[active].text}</small>}</>:captions.cues[active]?.text}</div>}{(isFullscreen||isPseudoFullscreen)&&<button className="custom-fullscreen" title="Έξοδος από πλήρη οθόνη" aria-label="Έξοδος από πλήρη οθόνη" onClick={()=>void toggleFullscreen()}>↙</button>}</div>
              <div className="player-actions">
                <div className="player-toolbar">
                  <div className="playback-controls" role="group" aria-label="Έλεγχος αναπαραγωγής"><button aria-label="Πίσω 10 δευτερόλεπτα" onClick={()=>skip(-10)}>↶ <span>10</span></button><button className="play-toggle" aria-label={isPlaying?"Παύση":"Αναπαραγωγή"} onClick={togglePlayback}>{isPlaying?"Ⅱ":"▶"}</button><button aria-label="Μπροστά 10 δευτερόλεπτα" onClick={()=>skip(10)}><span>10</span> ↷</button></div>
                  <div className="display-controls"><span className="subtitle-size-label">Μέγεθος υποτίτλων</span><div className="subtitle-size-picker" role="group" aria-label="Μέγεθος ελληνικών υποτίτλων">{[16,19,22].map((size,index)=><button key={size} className={state.settings.subtitleSize===size?"active":""} style={{fontSize:`${10+index*3}px`}} aria-label={index===0?"Μικροί υπότιτλοι":index===1?"Μεσαίοι υπότιτλοι":"Μεγάλοι υπότιτλοι"} aria-pressed={state.settings.subtitleSize===size} onClick={()=>setState(current=>({...current,settings:{...current.settings,subtitleSize:size,subtitleSizeVersion:2}}))}>A</button>)}</div><button className="fullscreen-toggle" aria-label="Full screen" onClick={()=>void toggleFullscreen()}><span aria-hidden="true">↗</span><b>Full screen</b></button></div>
                </div>
                <div className="player-secondary-actions"><button className="moment-save" onClick={()=>beginMoment()}><span aria-hidden="true">＋</span> Κράτησε στιγμή</button><button className="transcript-toggle" onClick={()=>setTranscriptOpen(value=>!value)}>{transcriptOpen?"Κλείσιμο μεταγραφής":"Άνοιγμα μεταγραφής"}</button></div>
              </div>
            </div>
...
function Brand({home}:{home:()=>void}){return <button className="brand brand-home" aria-label="Αρχική σελίδα" onClick={home}><span className="brand-mark"><i>≡</i>▶</span><span>GreekTube <b>Subs</b></span><small className="brand-version">ver 4.3</small></button>;}
...
function AddVideo({close,add,existingIds}:{close:()=>void;add:(v:Video,t:boolean)=>Promise<void>;existingIds:string[]}) {
  const [url,setUrl]=useState("");const [metadata,setMetadata]=useState<{id:string;title:string;originalTitle?:string;channel:string;duration?:number}|null>(null);const [busy,setBusy]=useState(false);const [error,setError]=useState("");
