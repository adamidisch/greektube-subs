...
export default function GreekTubePlayer() {
  ...
  useEffect(()=>{
    const key=(event:KeyboardEvent)=>{
      ...
      const target=player.current;
      if(!target||typeof target.getCurrentTime!=="function")return;
      if(event.code==="Space"){
        if(event.repeat)return;
        event.preventDefault();
