            ...
            <div className="sticky-player" onContextMenu={e=>{e.preventDefault();beginMoment();}}>
              ...
              <div className="player-actions">
                <div className="player-toolbar">
                  <section className="control-section playback-section"><small>Αναπαραγωγή</small><div className="playback-controls" role="group" aria-label="Έλεγχος αναπαραγωγής"><button aria-label="Πίσω 10 δευτερόλεπτα" onClick={()=>skip(-10)}>↶ <span>10</span></button><button className="play-toggle" aria-label={isPlaying?"Παύση":"Αναπαραγωγή"} onClick={togglePlayback}>{isPlaying?"Ⅱ":"▶"}</button><button aria-label="Μπροστά 10 δευτερόλεπτα" onClick={()=>skip(10)}><span>10</span> ↷</button></div></section>
                  <section className="control-section display-section"><small>Υπότιτλοι και προβολή</small><div className="display-controls"><span className="subtitle-size-label">Μέγεθος</span><div className="subtitle-size-picker" role="group" aria-label="Μέγεθος ελληνικών υποτίτλων">{[16,19,22].map((size,index)=><button key={size} className={state.settings.subtitleSize===size?"active":""} style={{fontSize:`${10+index*3}px`}} aria-label={index===0?"Μικροί υπότιτλοι":index===1?"Μεσαίοι υπότιτλοι":"Μεγάλοι υπότιτλοι"} aria-pressed={state.settings.subtitleSize===size} onClick={()=>setState(current=>({...current,settings:{...current.settings,subtitleSize:size,subtitleSizeVersion:2}}))}>A</button>)}</div><button className="fullscreen-toggle" aria-label="Πλήρης οθόνη" onClick={()=>void toggleFullscreen()}><span aria-hidden="true">↗</span><b>Πλήρης οθόνη</b></button></div></section>
                </div>
                <section className="control-section action-section"><small>Ενέργειες</small><div className="player-secondary-actions"><button className="moment-save" onClick={()=>beginMoment()}><span aria-hidden="true">＋</span> Αποθήκευση στιγμής</button><button className="transcript-toggle" onClick={()=>setTranscriptOpen(value=>!value)}>{transcriptOpen?"Κλείσιμο μεταγραφής":"Άνοιγμα μεταγραφής"}</button></div></section>
              </div>
            </div>
            <div className="video-heading"><div><small>{selected.channel} · {CATEGORY_LABELS[selected.category]} · Προβολές: {selected.views||0}</small><h1 className="player-greek-title">{isGreekTitle(selected.title)?selected.title:isGreekTitle(captions.title)?captions.title:"Βίντεο με ελληνικούς υπότιτλους"}</h1>{(selected.originalTitle||captions.originalTitle||englishTitle(selected))&&<p className="player-original-title">{selected.originalTitle||captions.originalTitle||englishTitle(selected)}</p>}<div className="speaker-row"><span>Ομιλητής</span><strong>{selected.speakerName||speaker.name}</strong><i>{speaker.role}</i></div></div><div className="heading-actions"><button className="edit-video" onClick={()=>setEditingVideo(selected)}>Επεξεργασία</button><button aria-label="Αγαπημένο" className={`favorite ${selected.favorite?"active":""}`} onClick={()=>patchVideo(selected.id,{favorite:!selected.favorite})}>♥</button></div></div>
...
function Brand({home}:{home:()=>void}){return <button className="brand brand-home" aria-label="Αρχική σελίδα" onClick={home}><span className="brand-mark"><i>≡</i>▶</span><span>GreekTube <b>Subs</b></span><small className="brand-version">ver 4.6</small></button>;}
...
function EditVideo({video,close,save}:{video:Video;close:()=>void;save:(patch:Partial<Video>)=>void}) {
