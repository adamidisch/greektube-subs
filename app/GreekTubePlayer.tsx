      ...
      {!loading&&captions&&<>
        <section className={`watch-layout ${transcriptOpen?"transcript-open":"player-only"}`}>
          ...
          {transcriptOpen&&<aside className="side-panel transcript-drawer">
            <div className="drawer-header"><div><small>ΕΛΛΗΝΙΚΟΙ ΥΠΟΤΙΤΛΟΙ</small><strong>Μεταγραφή</strong></div><button aria-label="Κλείσιμο μεταγραφής" onClick={()=>setTranscriptOpen(false)}>×</button></div>
...
function Brand({home}:{home:()=>void}){return <button className="brand brand-home" aria-label="Αρχική σελίδα" onClick={()=>{home();window.location.assign("/");}}><span className="brand-mark"><i>≡</i>▶</span><span>GreekTube <b>Subs</b></span><small className="brand-version">ver 5.13</small></button>;}
