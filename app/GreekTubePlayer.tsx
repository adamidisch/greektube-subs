...
function isCompleteGreekTranscript(data:Captions|null|undefined,duration=0) {
  if(!data?.cues?.length||data.transcriptVersion!==10)return false;
...
export default function GreekTubePlayer() {
  ...
  async function syncLocalTranscripts(videos:Video[]){
    ...
    for(const video of videos){
      try{
        const raw=localStorage.getItem(`greektube-transcript:${video.id}:v10`);
        if(!raw)continue;
  ...
  async function openVideo(video:Video,start?:number,showTranscript=false,forceTranslation=false){
      ...
      try{
        ...
        if(readyResponse.ok){
          ...
          if(isCompleteGreekTranscript(ready,video.duration)){
            localStorage.setItem(`greektube-transcript:${video.id}:v10`,JSON.stringify(ready));
    ...
    const localRecord=localStorage.getItem(`greektube-transcript:${video.id}:v10`);
      ...
      try{
        ...
        if(isCompleteGreekTranscript(cached,video.duration)){
          ...
          void fetch("/api/captions",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:video.url,cachedTranscript:cached})}).then(async response=>{
            ...
            localStorage.setItem(`greektube-transcript:${video.id}:v10`,JSON.stringify(refreshed));
    ...
    try{
      let data:Captions;
      {
        ...
        data=sharedData;
        localStorage.setItem(`greektube-transcript:${video.id}:v10`,JSON.stringify(sharedData));
      ...
      const local=localStorage.getItem(`greektube-transcript:${video.id}:v10`);
      if(local){
        try{
  ...
  async function rebuildTranslation(video:Video){
    ...
    localStorage.removeItem(`greektube-transcript:${video.id}:v10`);
...
function Brand({home}:{home:()=>void}){return <button className="brand brand-home" aria-label="Αρχική σελίδα" onClick={home}><span className="brand-mark"><i aria-hidden="true"/>▶</span><span>GreekTube <b>Subs</b></span><small className="brand-version">ver 7.1.7</small></button>;}
