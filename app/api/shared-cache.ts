...
const TRANSLATE_DURABLE_BATCH = 12;
...
export function ensureTranscriptTable() {
  ...
  transcriptTableReady = (async () => {
    ...
    `CREATE TABLE IF NOT EXISTS video_transcripts (
      ...
      raw_english_count INTEGER NOT NULL DEFAULT 0,
      english_count INTEGER NOT NULL DEFAULT 0,
      greek_count INTEGER NOT NULL DEFAULT 0,
    ...
    await db.query("CREATE INDEX IF NOT EXISTS video_transcripts_status_idx ON video_transcripts (status, updated_at)");
  })().catch((error: unknown) => {
...
type Row = {
  ...
  processing_started_at: string | null; error: string | null; transcript_version: number;
  created_at: string; updated_at: string; raw_english_count: number; english_count: number; greek_count: number;
};
...
type LegacyCountRow = { raw_count: number | string; english_count: number | string; greek_count: number | string };
...
async function payloadForRow(row: Row) {
  ...
  // Legacy bootstrap only. Before transferring any large TEXT column, verify on
  // the database server that those legacy arrays still match the small counters.
  // New Blob-primary checkpoints intentionally leave legacy TEXT untouched, so
  // stale fallback data can never silently overwrite a newer Blob checkpoint.
  const db = database();
  const countRows = await db.query(
    `SELECT
       jsonb_array_length(COALESCE(NULLIF(raw_english_transcript, ''), '[]')::jsonb) AS raw_count,
  ...
  if (!counts || Number(counts.raw_count) !== Number(row.raw_english_count) ||
      Number(counts.english_count) !== Number(row.english_count) ||
      Number(counts.greek_count) !== Number(row.greek_count)) {
    console.error("[transcript-cache:blob-required]", JSON.stringify({ videoId: row.video_id }));
    return null;
  }
  ...
  return await publishTranscriptCheckpoint(row.video_id, row.transcript_version, snapshot) ? snapshot : null;
}
...
function normalizeReadyTranscriptOrder(
  ...
  return {
    ...
    englishTranscript: englishTranscript.length === greekTranscript.length ? order.map(index => englishTranscript[index]) : englishTranscript,
    timestamps: timestamps.length === greekTranscript.length ? order.map(index => timestamps[index]) : timestamps,
  };
}
...
export async function getTranscript(videoId: string) {
  ...
  const rows = await db.query(
    `SELECT video_id, title, channel, thumbnail, duration, original_language,
      ...
      created_at, updated_at, raw_english_count, english_count, greek_count
     FROM video_transcripts WHERE video_id = $1 LIMIT 1`,
    [videoId],
  ) as Row[];
  const row = rows[0];
  ...
  return {
    ...
    englishTranscript: readyOrder?.englishTranscript || storedEnglishTranscript,
    greekTranscript: readyOrder?.greekTranscript || storedGreekTranscript,
    timestamps: readyOrder?.timestamps || storedTimestamps,
...
type StatusRow = {
  video_id: string; status: TranscriptRecord["status"]; progress: number; lock_expires_at: string | null;
  ...
  raw_english_count: number | string; english_count: number | string; greek_count: number | string; key_points: string;
};
...
export async function getTranscriptStatus(videoId: string): Promise<TranscriptStatusRecord | null> {
  ...
  const db = database();
  const rows = await db.query(
    `SELECT video_id, status, progress, lock_expires_at, processing_stage, processing_cursor,
      ...
      raw_english_count, english_count, greek_count
     FROM video_transcripts WHERE video_id = $1 LIMIT 1`,
    [videoId],
  ) as StatusRow[];
  const row = rows[0];
...
export async function acquireProcessingLock(videoId: string, token: string, force = false) {
  ...
  const rows = await db.query(
    ...
    ON CONFLICT(video_id) DO UPDATE SET
      status = 'processing',
      progress = CASE WHEN $8 = 1 OR video_transcripts.transcript_version != EXCLUDED.transcript_version THEN 3 ELSE GREATEST(video_transcripts.progress, 3) END,
      processing_stage = CASE WHEN $8 = 1 OR video_transcripts.transcript_version != EXCLUDED.transcript_version THEN NULL ELSE video_transcripts.processing_stage END,
      processing_cursor = CASE WHEN $8 = 1 OR video_transcripts.transcript_version != EXCLUDED.transcript_version THEN 0 ELSE video_transcripts.processing_cursor END,
      retry_count = CASE WHEN $8 = 1 OR video_transcripts.transcript_version != EXCLUDED.transcript_version OR video_transcripts.status = 'failed' THEN 0 ELSE video_transcripts.retry_count END,
      retry_after = NULL,
      processing_started_at = CASE WHEN $8 = 1 OR video_transcripts.transcript_version != EXCLUDED.transcript_version THEN EXCLUDED.processing_started_at ELSE COALESCE(video_transcripts.processing_started_at, EXCLUDED.processing_started_at) END,
    ...
    WHERE $8 = 1 OR video_transcripts.transcript_version != EXCLUDED.transcript_version OR video_transcripts.status = 'failed'
       OR video_transcripts.lock_expires_at IS NULL OR video_transcripts.lock_expires_at < EXCLUDED.updated_at
    RETURNING video_id`,
...
export async function completeTranscript(record: TranscriptRecord, token: string) {
  ...
  if (!await publishTranscriptCheckpoint(record.videoId, record.transcriptVersion, snapshot)) return false;

  const db = database();
  const rows = await db.query(
    `UPDATE video_transcripts SET
      title=$1, channel=$2, thumbnail=$3, duration=$4, original_language=$5,
      raw_english_count=$6, english_count=$7, greek_count=$8, topics=$9, key_points=$10,
      status='ready', progress=100, lock_token=NULL, lock_expires_at=NULL, error=NULL, retry_count=0, retry_after=NULL,
      processing_stage=NULL, processing_cursor=0, transcript_version=$11, updated_at=$12
     WHERE video_id=$13 AND lock_token=$14 RETURNING video_id`,
    [record.title, record.channel, record.thumbnail, record.duration, record.originalLanguage,
      record.rawEnglishTranscript.length, record.englishTranscript.length, record.greekTranscript.length,
      JSON.stringify(record.topics), JSON.stringify(record.keyPoints), record.transcriptVersion, record.updatedAt, record.videoId, token],
  ) as { video_id: string }[];
  return rows.length === 1;
}
...
async function heartbeatProcessingLock(videoId: string, token: string) {
  const db = database();
  const rows = await db.query(
    "UPDATE video_transcripts SET lock_expires_at=$1, updated_at=$2 WHERE video_id=$3 AND lock_token=$4 RETURNING video_id",
    [new Date(Date.now()+180_000).toISOString(), new Date().toISOString(), videoId, token],
  ) as { video_id: string }[];
  return rows.length === 1;
}
...
export async function saveProcessingCheckpoint(videoId: string, token: string, checkpoint: ProcessingCheckpoint) {
  ...
  const hasPayload = checkpoint.rawEnglishTranscript !== undefined || checkpoint.englishTranscript !== undefined || checkpoint.greekTranscript !== undefined;

  // Translation providers may accept cues one-by-one inside a request. Persist
  // only complete 12-cue boundaries (or the final cue) so Blob operations stay
  // bounded and a failed request simply replays one small batch.
  if (stage === "translate" && hasPayload && checkpoint.progress < 90 && checkpoint.cursor % TRANSLATE_DURABLE_BATCH !== 0) {
    return checkpoint.cursor % 6 === 0 ? heartbeatProcessingLock(videoId, token) : true;
  }

  const now = new Date().toISOString();
  const current = await readTranscriptCheckpoint(videoId, TRANSCRIPT_VERSION);
  const needsBlobWrite = hasPayload || !current || current.processingStage !== checkpoint.stage || current.processingCursor !== checkpoint.cursor;
  ...
  if (needsBlobWrite) {
    ...
    snapshot = await upsertTranscriptCheckpoint(videoId, TRANSCRIPT_VERSION, patch);
  ...
  const rows = await db.query(
    `UPDATE video_transcripts SET
      ...
      raw_english_count=$4, english_count=$5, greek_count=$6,
      title=COALESCE($7,title), channel=COALESCE($8,channel), duration=COALESCE($9,duration), original_language=COALESCE($10,original_language),
      updated_at=$11, lock_expires_at=$12
     WHERE video_id=$13 AND lock_token=$14 RETURNING video_id`,
    [checkpoint.stage, checkpoint.cursor, checkpoint.progress,
      snapshot.rawEnglishTranscript.length, snapshot.englishTranscript.length, snapshot.greekTranscript.length,
...
export async function resetProcessingForTranslation(videoId: string, token: string, keepRaw = true) {
  const current = await readTranscriptCheckpoint(videoId, TRANSCRIPT_VERSION);
  if (keepRaw && !current?.rawEnglishTranscript.length) return false;
  const now = new Date().toISOString();
  const snapshot = await upsertTranscriptCheckpoint(videoId, TRANSCRIPT_VERSION, {
    status: "processing",
    processingStage: keepRaw ? "repair" : "source",
    processingCursor: 0,
    rawEnglishTranscript: keepRaw ? (current?.rawEnglishTranscript || []) : [],
    englishTranscript: [],
    greekTranscript: [],
    timestamps: [],
    updatedAt: now,
  ...
  const rows = await db.query(
    `UPDATE video_transcripts SET status='processing', progress=$1, processing_stage=$2, processing_cursor=0,
      raw_english_count=$3, english_count=0, greek_count=0, topics='[]', key_points='[]',
      error=NULL, retry_count=0, retry_after=NULL, updated_at=$4
     WHERE video_id=$5 AND lock_token=$6 RETURNING video_id`,
    [keepRaw ? 28 : 3, keepRaw ? 'repair' : 'source', snapshot.rawEnglishTranscript.length, now, videoId, token],
  ) as { video_id: string }[];
  return rows.length === 1;
}
...
export async function recordGroqRateLimit(videoId: string, token: string, retryAfterSeconds?: number) {
  ...
  const rows = await db.query(
    `UPDATE video_transcripts SET groq_429_streak=groq_429_streak+1,
      groq_cooldown_until=CASE WHEN groq_429_streak+1>=2 THEN CASE WHEN groq_cooldown_until IS NOT NULL AND groq_cooldown_until>$1 THEN groq_cooldown_until ELSE $1 END ELSE groq_cooldown_until END,
...
export async function recordGroqProviderSuccess(videoId: string, token: string) {
  const db = database();
  const rows = await db.query(
    `UPDATE video_transcripts SET groq_429_streak=0, groq_cooldown_until=NULL, updated_at=$1, lock_expires_at=$2
     WHERE video_id=$3 AND lock_token=$4 RETURNING video_id`,
...
export async function recordTransientProcessingFailure(videoId: string, token: string, message: string) {
  ...
  const rows = await db.query(
    `UPDATE video_transcripts SET retry_count=retry_count+1, retry_after=$1, error=$2,
     ...
     WHERE video_id=$5 AND lock_token=$6 RETURNING status, retry_count, retry_after`,
    [retryAfter, message.slice(0,500), now.toISOString(), MAX_TRANSIENT_RETRIES, videoId, token],
  ) as { status: TranscriptRecord["status"]; retry_count: number; retry_after: string | null }[];
  if (rows[0]?.status === "failed") await upsertTranscriptCheckpoint(videoId, TRANSCRIPT_VERSION, { status: "failed", updatedAt: now.toISOString() });
  return rows[0] || null;
}
...
export async function failTranscript(videoId: string, token: string, message: string) {
  ...
  const now = new Date().toISOString();
  const rows = await db.query(
    `UPDATE video_transcripts SET status='failed', error=$1, lock_token=NULL, lock_expires_at=NULL, updated_at=$2
     WHERE video_id=$3 AND lock_token=$4 RETURNING video_id`,
    [message.slice(0,500), now, videoId, token],
  ) as { video_id: string }[];
  if (rows.length === 1) await upsertTranscriptCheckpoint(videoId, TRANSCRIPT_VERSION, { status: "failed", updatedAt: now });
}