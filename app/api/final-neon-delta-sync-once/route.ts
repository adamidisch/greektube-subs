...
async function rowsForCopy(client: { query: (sql: string, params?: unknown[]) => Promise<{ rows: unknown[] }> }, table: string, orderBy: string) {
  ...
  return (await client.query(`SELECT * FROM ${table} ORDER BY ${orderBy}`)).rows;
}

async function nativeHash(client: { query: (sql: string, params?: unknown[]) => Promise<{ rows: unknown[] }> }, table: string, orderBy: string) {
...
export async function POST(request: Request) {
  ...
  if (!sourceUrl || !targetUrl) {
    return NextResponse.json({ ok: false, error: "database-url-missing" }, { status: 500 });
  }
  if (sourceUrl === targetUrl) {
    return NextResponse.json({ ok: false, error: "source-and-target-are-identical" }, { status: 500 });
  }
  ...
  try {
    ...
    for (const [table, orderBy] of TABLES) {
      ...
      const sourceCheck = await nativeHash(source, table, orderBy);
      ...
      const targetCheck = await nativeHash(target, table, orderBy);
    ...
    return NextResponse.json({ ok: true, parity: true, counts, hashes }, {
      headers: { "Cache-Control": "private, no-store", "X-Robots-Tag": "noindex" },
    });
  } catch (error) {
    await target.query("ROLLBACK").catch(() => undefined);
    await source.query("ROLLBACK").catch(() => undefined);
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : String(error), counts }, {
      status: 500,
      headers: { "Cache-Control": "private, no-store", "X-Robots-Tag": "noindex" },
    });
  } finally {
