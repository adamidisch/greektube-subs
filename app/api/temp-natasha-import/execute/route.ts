...
type Checkpoint = {
  transcriptVersion?: unknown; status?: unknown; processingStage?: unknown; processingCursor?: unknown;
  rawEnglishTranscript?: unknown; englishTranscript?: unknown; greekTranscript?: unknown; timestamps?: unknown;
};
...
export async function GET() {
  ...
  const currentCheckpoint = await checkpointText();
  const current = JSON.parse(currentCheckpoint) as Checkpoint;
  const currentEnglish = Array.isArray(current.englishTranscript) ? current.englishTranscript as Cue[] : [];
  const currentGreek = Array.isArray(current.greekTranscript) ? current.greekTranscript as Cue[] : [];
  if (before.status === "ready" && Number(before.progress) === 100 && Number(before.english_count) === EXPECTED_CUES && Number(before.greek_count) === EXPECTED_CUES &&
      current.status === "ready" && current.transcriptVersion === TRANSCRIPT_VERSION && currentEnglish.length === EXPECTED_CUES && currentGreek.length === EXPECTED_CUES &&
      canonicalCueHash(currentEnglish) === EXPECTED_SOURCE_HASH && hashCues(currentGreek) === greekHash) {
    const result = { ok: true, alreadyImported: true, videoId: VIDEO_ID, sourceHash: EXPECTED_SOURCE_HASH, greekHash, englishCount: EXPECTED_CUES, greekCount: EXPECTED_CUES };
    console.info("[natasha-owner-import:already-ready]", JSON.stringify(result));
    return NextResponse.json(result);
  }

  const rawEnglish = Array.isArray(current.rawEnglishTranscript) ? current.rawEnglishTranscript as Cue[] : [];
  if (!rawEnglish.length) throw new Error("Raw English backup is empty");
  await put(BACKUP_PATH, currentCheckpoint, { access: "public", addRandomSuffix: false, allowOverwrite: true, cacheControlMaxAge: 60, contentType: "application/json; charset=utf-8" });

  try {
    const lockToken = crypto.randomUUID();
    if (!await acquireProcessingLock(VIDEO_ID, lockToken, true)) throw new Error("Natasha import lock failed");
    const now = new Date().toISOString();
    const duration = Math.max(Number(before.duration) || 0, greekCues.reduce((max, cue) => Math.max(max, cue.start + cue.duration), 0));
    ...
    const record = {
      videoId: VIDEO_ID, title: before.title || "Let Food Be Thy Medicine", channel: before.channel || "YouTube",
      thumbnail: before.thumbnail || `https://i.ytimg.com/vi/${VIDEO_ID}/hqdefault.jpg`, duration,
      originalLanguage: before.original_language || "en", rawEnglishTranscript: rawEnglish, englishTranscript: canonical.cues,
      greekTranscript: greekCues, timestamps: canonical.cues.map(cue => ({ start: cue.start, duration: cue.duration })),
      topics, keyPoints, status: "ready" as const, progress: 100, transcriptVersion: TRANSCRIPT_VERSION,
      createdAt: before.created_at || now, updatedAt: now,
    };
    ...
    const neonRows = await db.query(
      `SELECT status, progress, transcript_version, raw_english_count, english_count, greek_count, processing_stage, processing_cursor, updated_at
       FROM video_transcripts WHERE video_id=$1 LIMIT 1`, [VIDEO_ID],
    ...
    if (!neon || neon.status !== "ready" || Number(neon.progress) !== 100 || Number(neon.transcript_version) !== TRANSCRIPT_VERSION ||
        ...
        neon.processing_stage !== null || Number(neon.processing_cursor) !== 0) throw new Error(`Neon read-back failed: ${JSON.stringify(neon)}`);

    const saved = JSON.parse(await checkpointText()) as Checkpoint;
    ...
    if (saved.status !== "ready" || saved.transcriptVersion !== TRANSCRIPT_VERSION || saved.processingStage !== null || Number(saved.processingCursor) !== 0 ||
        ...
        blobEnglishHash !== EXPECTED_SOURCE_HASH || blobGreekHash !== greekHash) throw new Error(`Blob read-back failed: ${JSON.stringify({ status:saved.status, version:saved.transcriptVersion, raw:savedRaw.length, english:savedEnglish.length, greek:savedGreek.length, timestamps:savedTimestamps.length, blobEnglishHash, blobGreekHash })}`);

    const result = { ok: true, videoId: VIDEO_ID, transcriptVersion: TRANSCRIPT_VERSION, sourceHash: EXPECTED_SOURCE_HASH, greekHash,
      rawEnglishCount: savedRaw.length, englishCount: savedEnglish.length, greekCount: savedGreek.length, timestampCount: savedTimestamps.length,
    ...
    await restore(db, before, currentCheckpoint).catch(rollbackError => console.error("[natasha-owner-import:rollback-failed]", rollbackError));
    throw error;
  }
}
