...
export default function UserProfileEnhancer() {
    ...
    void (async () => {
      for (let attempt = 0; attempt < 6 && !cancelled; attempt += 1) {
        try {
          ...
          if (!cancelled && data.profile?.username) setProfile(data.profile);
          return;
        } catch {
          await delay(700);
        }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void fetch(`/api/profile?check=${encodeURIComponent(candidate)}`, { credentials: "same-origin", cache: "no-store" })
        .then(async response => {
  ...
  async function saveUsername(event: FormEvent<HTMLFormElement>) {
    ...
    if (!validUsername(candidate) || availability !== "available") return;
    setSaving(true);
    try {
  ...
  const profileUi = <div className="user-profile-shell" ref={shellRef}>
    ...
    {menuOpen && <div className="user-profile-menu" role="menu">
      ...
      <button type="button" className="user-menu-row" role="menuitem" onClick={() => setEditorOpen(true)}>
    ...
    <section className="profile-editor" role="dialog" aria-modal="true" aria-label="Αλλαγή ονόματος χρήστη">
      ...
      <form onSubmit={saveUsername}>
        ...
        <div className={`profile-name-field ${availability}`}>
          <span className="user-avatar"><UserIcon/></span>
          <input id="greektube-username" autoFocus autoComplete="off" maxLength={24} value={draft} onChange={event => setDraft(event.target.value)} />
        </div>
        ...
        <div className="profile-editor-actions"><button type="button" className="secondary" onClick={() => setEditorOpen(false)} disabled={saving}>Ακύρωση</button><button type="submit" className="primary" disabled={saving || (draft.trim() !== profile.username && availability !== "available")}>{saving ? "Αποθήκευση…" : "Αποθήκευση"}</button></div>
      </form>
    </section>
