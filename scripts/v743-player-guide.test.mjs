...
const detailsCss=fs.readFileSync('app/v7-4-9.css','utf8');
...
const checks=[
  ...
  ['version',pkg.version==='7.4.12'&&layout.includes('"app-version": "7.4.12"')&&layout.includes('import "./v7-4-9.css";')],
...
for(const [label,ok] of checks){if(!ok)throw new Error(`v7.4.12 invariant failed: ${label}`)}
console.log(`v7.4.12 player/guide invariants passed (${checks.length})`);
